...
if [[ -z "$http" ]]; then
  ...
  echo -e "Your IP/DNS Address is ${wanip}"
  echo -e "Your public key is ${key}"
  echo -e "Install Rustdesk:"
  echo -e "\`pikaur -S rustdesk\`"
  echo -e "Replace \`secure-string\` in ~/Downloads/rustdeskclientinstall.sh with:"
  echo "$string64rev"
  echo -E "Configure Network Settings in Rustdesk Client to use your server details and public key (leave other fields blank)"
fi
